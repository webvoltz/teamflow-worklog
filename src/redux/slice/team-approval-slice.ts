...
export const fetchTeamWorkPlans = createAsyncThunk<TeamWorkPlanEntry[], { teamLeaderId: string }, { rejectValue: QueryError }>(
    "query/fetchTeamWorkPlans",
...
>("mutation/reviewWorkPlan", async ({ entryId, status, note }, { rejectWithValue }) => {
    try {
        ...
        if (!data?.reviewWorkPlan?.success) {
            return rejectWithValue({ message: data?.reviewWorkPlan?.message ?? "Unable to update work plan." });
        }
...
const teamApprovalSlice = createSlice({
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(fetchTeamWorkPlans.fulfilled, (state, action: PayloadAction<TeamWorkPlanEntry[]>) => {
                state.loading = false;
