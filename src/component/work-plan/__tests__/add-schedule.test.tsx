...
describe('Add work plan', () => {
  ...
  it('lets an employee add and submit a new work schedule', async () => {
    ...
    // No sample data ships pre-seeded, so the "Add new schedule" flow is exercised
    // from a clean slate rather than needing to clear an existing entry first.
    ...
    await waitFor(() => {
      expect(store.getState().projectOption.data).not.toBeNull();
    });
